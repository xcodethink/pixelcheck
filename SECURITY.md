...
`@ai-sdk/provider-utils@3.0.30`, not a range. The resolved version is whatever
that `ai` release chose, so overriding one package cannot change it without
breaking the version agreement the whole family is built on.
...
is a `3.1.0` prerelease. The advisory's `<=3.0.97` is a generous ceiling over
a line abandoned before it was ever fixed: the fix landed in the 4.x and 5.x
lines, which belong to the newer `ai` generations stagehand does not accept.
...
content reaches the model as prompt text and never reaches this parser.
